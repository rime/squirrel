...
- (void)multilineRectForRange:(NSRange)charRange leadingRect:(NSRect *)leadingRect bodyRect:(NSRect *)bodyRect trailingRect:(NSRect *)trailingRect {
  ...
  NSRect firstLineRect = [layoutManager lineFragmentUsedRectForGlyphAtIndex:glyphRange.location effectiveRange:&firstLineRange];
  NSRange lastLineRange = NSMakeRange(NSNotFound, 0);
  NSRect lastLineRect = [layoutManager lineFragmentUsedRectForGlyphAtIndex:NSMaxRange(glyphRange)-1 effectiveRange:&lastLineRange];
  ...
  *trailingRect = NSZeroRect;
  ...
  if (boundingRect.origin.x <= 1 && firstLineRange.location < glyphRange.location) {
    *leadingRect = [layoutManager boundingRectForGlyphRange:NSMakeRange(firstLineRange.location, glyphRange.location-firstLineRange.location) inTextContainer:textContainer];
  ...
  if (NSMaxRange(lastLineRange) > NSMaxRange(glyphRange)) {
    *trailingRect = [layoutManager boundingRectForGlyphRange:
                    NSMakeRange(NSMaxRange(glyphRange), NSMaxRange(lastLineRange)-NSMaxRange(glyphRange))
